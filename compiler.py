...
class Comp_RV32I(BaseCompiler):
    ...
    def decompile_instr(binary: str): # override
        ...
        match instr_type:
            ...
            case 'I':
                ...
                imm = BaseCompiler.twos_to_int(binary[0:12])
                if type(instr) != str:
                    if instr[0] == 0:
            ...
            case 'S':
                imm = BaseCompiler.twos_to_int(binary[:7] + binary[20:25])
                return args1[2][funct3], rs1, rs2, imm
            case 'B':
                imm = BaseCompiler.twos_to_int(binary[0] + binary[24] + binary[1:7] + binary[20:24] + '0')
                return args1[2][funct3], rs1, rs2, imm
            case 'U':
                return args1[0], rd, (BaseCompiler.twos_to_int(binary[:20]) << 12)
            case 'J':
                imm = BaseCompiler.twos_to_int(binary[0] + binary[12:20] + binary[11] + binary[1:11] + '0')
    ...
    def compile_R(opcode, rd, funct3, rs1, rs2, funct7):
        return ''.join(
            [BaseCompiler.twos_comp_bin(*i) for i in [(funct7, 7), (rs2, 5), (rs1, 5), (funct3, 3), (rd, 5), (opcode, 7)]])

    @staticmethod
    def compile_I(opcode, rd, funct3, rs1, imm):
        if not -2048 <= imm <= 2047:
            raise CompilerError(f"Invalid immediate value for I-type instr: {imm}")
        return ''.join([BaseCompiler.twos_comp_bin(*i) for i in [(imm, 12), (rs1, 5), (funct3, 3), (rd, 5), (opcode, 7)]])

    @staticmethod
    def compile_S(opcode, funct3, rs1, rs2, imm):
        if not -2048 <= imm <= 2047:
            raise CompilerError(f"Invalid immediate value for S-type instr: {imm}")
        imm_bin = BaseCompiler.twos_comp_bin(imm, bits=12)
        return (imm_bin[:7] + ''.join([BaseCompiler.twos_comp_bin(*i) for i in [(rs2, 5), (rs1, 5), (funct3, 3)]])
                + imm_bin[7:] + BaseCompiler.twos_comp_bin(opcode, 7))

    @staticmethod
    def compile_B(opcode, funct3, rs1, rs2, imm):
        if not imm % 2 == 0:
            raise CompilerError(f"Invalid immediate value for B-type instr: {imm}\nValue should be an even number.")
        imm = imm // 2
        if not -2048 <= imm <= 2047:
            raise CompilerError(f"Invalid immediate value for B-type instr: {imm}")
        imm_bin = BaseCompiler.twos_comp_bin(imm, bits=12)
        return imm_bin[0] + imm_bin[2:8] + ''.join(
            [BaseCompiler.twos_comp_bin(*i) for i in [(rs2, 5), (rs1, 5), (funct3, 3)]]) + imm_bin[8:] + imm_bin[
            1] + BaseCompiler.twos_comp_bin(opcode, 7)

    @staticmethod
    def compile_U(opcode, rd, imm):
        if not (imm >> 12) << 12 == imm:
            raise CompilerError(f"Invalid immediate value for U-type instr: {imm}\nValue should be a multiple of 2^12")
        imm = imm >> 12
        if not -524288 <= imm <= 524287:
            raise CompilerError(f"Invalid immediate value for U-type instr: {imm}")
        return ''.join([BaseCompiler.twos_comp_bin(*i) for i in [(imm, 20), (rd, 5), (opcode, 7)]])

    @staticmethod
    def compile_J(opcode, rd, imm):
        if not imm % 2 == 0:
            raise CompilerError(f"Invalid immediate value for J-type instr: {imm}\nValue should be an even number.")
        imm = imm >> 1
        if not -524288 <= imm <= 524287:
            raise CompilerError(f"Invalid immediate value for J-type instr: {imm}")
        imm_bin = BaseCompiler.twos_comp_bin(imm, bits=20)
        return imm_bin[0] + imm_bin[10:] + imm_bin[9] + imm_bin[1:9] + ''.join(
            [BaseCompiler.twos_comp_bin(*i) for i in [(rd, 5), (opcode, 7)]])
    ...
    def compile_instr(instr): # override
        ...
        def reg(x):
            if type(x) is int:
                ...
                val = int(x[1:])
            if not 0 <= val <= 31:
                raise CompilerError(f"Invalid reg value: {x}")
            return val
        ...
        match instr_type:
            ...
            case _:
                raise CompilerError(f"Invalid instruction type: {instr_type}")